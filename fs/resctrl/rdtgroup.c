...
static void mb_max_lim_init(void)
{
	...
	if (!ctrl->membw.arch_has_mb_max_lim)
		return;
				       ...
				       struct rdt_resource_final *f)
{
	...
	for_each_resource_ctrl(ctrl, f->res) {
		...
		/*
		 * In legacy mode an emulated control is nested under the
		 * default control it emulates. In native mode there is no
					  ...
					  char *buf, size_t nbytes, loff_t off)
{
	...
	if (mode != r->mode) {
		...
		if (ret) {
			 ...
			 */
			r->mode = old_mode;
			if (resctrl_ctrl_rebuild_subdirs(rdt_kn_parent(of->kn), f)) {
				/*
				 ...
				 */
				pr_err("%s: failed to restore resource_schemata after mode change error, control subdirectories may be missing\n",
				       f->name);
				rdt_last_cmd_puts("Failed to rebuild resource_schemata, filesystem may be inconsistent\n");
				ret = -EIO;
			} else {
				/*
				 ...
				 */
				rdt_last_cmd_puts("Failed to switch mode, reverted to previous mode\n");
			}
		}
	}
